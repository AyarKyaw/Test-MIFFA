...
<div class="p-4 p-md-5">
    ...
    <form id="quizForm" action="{{ route('courses.lessons.submit', [$course->id, $lesson->id]) }}" method="POST">
        @csrf
        @forelse($lesson->questions as $qIndex => $question)
            <div class="quiz-step" data-step="{{ $qIndex }}" style="{{ $qIndex !== 0 ? 'display: none;' : '' }}">
                ...
                <input type="hidden" name="step_index" value="{{ $qIndex }}">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 1. Radio Selection Styling & Activation
    ...
    // 2. Action Button Handling (Submit -> Next)
    quizForm.addEventListener('click', async function (e) {
        ...
        const stepIndex = parseInt(currentStep.getAttribute('data-step'));
        ...
        if (currentState === 'submit') {
            ...
            formData.append('question_id', currentStep.querySelector('input[name="question_id"]').value);
            ...
            try {
                ...
                // Transform button to "Next Question" or "Finish & See Results"
                actionBtn.removeAttribute('disabled');
        ...
        // STEP STATE B: Move to Next Step
        ...
        // STEP STATE C: Show final summary
